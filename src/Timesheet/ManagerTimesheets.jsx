...
const ManagerTimesheets = () => {
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      let url = `http://20.119.75.162:8086/api/timesheets/list/manager/${employeeId}`;
        
      if (startDate && endDate) {
        url = `http://20.119.75.162:8086/api/timesheets/totalList/employeeId/${employeeId}/startDate/${startDate}/endDate/${endDate}`;
      }
  ...
  const handleApprove = async (id) => {
    ...
    try {
      await axios.put(`http://20.119.75.162:8086/api/timesheets/Approve/${id}/status/APPROVED`, null, {
        headers: {
  ...
  const handleReject = async () => {
    setLoading(true);
    try {
      await axios.put(`http://20.119.75.162:8086/api/timesheets/reject/${currentId}/status/REJECTED/comments/${comments}`, null, {
        headers: {
