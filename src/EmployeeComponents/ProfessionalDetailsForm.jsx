...
const ProfessionalDetailsForm = ({ formData, onNext, onBack, onFormDataChange }) => {
    ...
    useEffect(() => {
        const fetchEmployees = async () => {
            ...
            try {
                const response = await fetch('http://20.119.75.162:8086/api/v1/employeeManager/employees', {
                    method: 'GET',
                    headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                ...
                const response = await fetch(`http://20.119.75.162:8086/api/v1/employeeManager/exists/${formData.employeeId}`, {
                    method: 'GET',
                    headers: {
