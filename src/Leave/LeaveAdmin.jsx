...
export default function LeaveApprovalDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://20.119.75.162:8086/api/leaves/manager/${managerId}`, {
          method:'GET',
          headers:{
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.put(`http://20.119.75.162:8086/api/leaves/approve/${id}`, null,  {
  ...
  const handleReject = async () => {
    ...
    const token = localStorage.getItem('token')
      await axios.put(`http://20.119.75.162:8086/api/leaves/reject/${selectedLeaveId}/${rejectionReason}`, null,  {
