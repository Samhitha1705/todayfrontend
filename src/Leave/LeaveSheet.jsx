...
const LeaveSheet = () => {
 ...
 const fetchLeaveSheet =  async () => {
  try{
    ...
    const response = await axios.get("http://20.119.75.162:8086/api/getSheets", {
        method:'GET',
        headers:{
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      ...
      if(isSubmitted && leaveId){
        const response =  await axios.put(`http://20.119.75.162:8086/api/updateSheet/${leaveId}`, leaveData, {
        ...
        setIsSubmitted(true); // Ensure it remains submitted
          ...
          const response = await axios.post(
            "http://20.119.75.162:8086/api/submitSheet",
            leaveData, {
