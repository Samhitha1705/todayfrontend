...
export default function LeaveEmployee() {
  ...
  if (!employeeId) {
    ...
    return;
      ...
      try {
        ...
        console.log(token)
        const response = await axios.get(`http://20.119.75.162:8086/api/leaves/employee/${employeeId}`, {
          headers:{
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token')
      await axios.delete(`http://20.119.75.162:8086/api/leaves/delete/${deleteRequestId}`, {
        method:'GET',
        headers:{
          'Authorization' : `Bearer ${token}`,
          'Content-Type' : 'application/json'
