        ...
        const fetchData = async () => {
          try {
            ...
            const [originResponse] = await Promise.all([
                axios.get(`http://20.119.75.162:8086/api/v1/employeeManager/origin/${employeeId}`, {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        ...
        const fetchRemainingLeaveDays = async (employeeId, leaveType, leaveStartDate, leaveEndDate) => {
            ...
            try {
                const token = localStorage.getItem('token')
                console.log(token)
                const response = await axios.get('http://20.119.75.162:8086/api/leaves/remaining-leaves', {
                    method:'GET',
                    headers:{
                      'Authorization' : `Bearer ${token}`,
                      'Content-Type' : 'application/json'
...
requiredFields.forEach(field => {
    console.log(`${field}: ${formData[field]}`);
        ...
        try {
            ...
            const url = isEditing
                ? `http://20.119.75.162:8086/api/leaves/update/${formData.id}`
                : `http://20.119.75.162:8086/api/leaves/submit`;
